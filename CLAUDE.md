...
- **Docker image publishing**: every push to `dev` builds and pushes `backend`/`viewer` images to GHCR (`ghcr.io/<repo>-backend`, `ghcr.io/<repo>-viewer`), tagged `dev` and `X.X.X-dev`. The version number comes from the `VERSION` file at the repo root — bump it by hand as part of the work that should carry a new number, not automatically.
