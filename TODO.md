...
- [ ] **Step 6 — Equipment.** CRUD, revision list, `auto_add` pre-selection in the activity form, usage aggregate, delete-vs-retire rule with the 409 path. → tag `v0.3.0`
- [ ] **Step 7 — Track parsing (client only).** `lib/tracks` with GPX and IGC parsers, stats and simplification, covered by Vitest against real sample files. Auto-fill in the create form. Nothing uploaded yet.
- [ ] **Step 8 — Track upload and map.** Blob store, upload and download endpoints, delete ordering, `FlightMap` with base layers and track rendering, fit-to-bounds. → tag `v0.4.0`
- [ ] **Step 9 — Altitude profile.** uPlot integration, shared cursor with the map, empty-profile variant, elevation service and `elevation.json` caching. → tag `v0.5.0`
